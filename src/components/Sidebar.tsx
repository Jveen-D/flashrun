...
export const Sidebar: React.FC<SidebarProps> = ({ onOpenSettings }) => {
  const { projects, activeProjectId, setActiveProject, addProject, isSidebarExpanded, setSidebarExpanded } = useStore();
  ...
  return (
    <aside className={`${isSidebarExpanded ? 'w-[260px]' : 'w-16'} h-screen bg-slate-50 dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col py-4 shrink-0 overflow-hidden select-none z-10 relative transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)]`}>
      <div className={`w-full flex items-center mb-5 shrink-0 transition-all duration-300 ${isSidebarExpanded ? 'px-5 justify-between' : 'px-0 justify-center flex-col gap-4'}`}>
        <div className={`flex items-center ${!isSidebarExpanded && 'justify-center'}`}>
          ...
          />
          <span className={`font-extrabold text-xl tracking-tight text-slate-800 dark:text-white overflow-hidden transition-all duration-300 whitespace-nowrap ${isSidebarExpanded ? 'w-auto opacity-100 ml-3' : 'w-0 opacity-0 ml-0'}`}>
            {t('FlashRun')}
          </span>
        </div>
        ...
        <button
          ...
          className="text-slate-400 hover:text-slate-700 dark:text-slate-500 dark:hover:text-white transition-colors shrink-0"
      ...
      <div className={`mb-4 shrink-0 transition-all duration-300 ${isSidebarExpanded ? 'px-5' : 'px-3'}`}>
        <div className="h-px bg-slate-200 dark:bg-slate-800 rounded-full w-full" />
      </div>

      <div className={`flex-1 w-full overflow-y-auto no-scrollbar flex flex-col space-y-1.5 pb-4 transition-all duration-300 ${isSidebarExpanded ? 'px-3' : 'px-2 items-center'}`}>
        {projects.map((project) => {
          ...
          return (
            <div key={project.id} className="relative group cursor-pointer w-full flex items-center justify-center" onClick={() => setActiveProject(project.id)}>
              ...
              <div className={`w-full flex items-center rounded-xl transition-all duration-200 border ${isSidebarExpanded ? 'p-2' : 'p-1 justify-center'} ${
                isActive
                ...
                <div
                  className={`shrink-0 flex items-center justify-center transition-all duration-200 relative ${isSidebarExpanded ? 'w-10 h-10' : 'w-11 h-11'} ${
                    isActive
                ...
                >
                  <FolderKanban size={isSidebarExpanded ? 20 : 22} strokeWidth={isActive ? 2.5 : 2} />
                ...
                <div className={`flex flex-col justify-center overflow-hidden transition-all duration-300 whitespace-nowrap ${
                  isSidebarExpanded ? 'ml-3 flex-1 min-w-0 opacity-100' : 'w-0 opacity-0 ml-0'
                }`}>
        ...
        <div className="w-full mt-2 pt-2 relative group flex justify-center">
          <button
            onClick={handleAddProject}
            className={`w-full flex items-center rounded-xl border border-dashed border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/30 text-emerald-600 dark:text-emerald-500 group hover:bg-emerald-50 dark:hover:bg-emerald-500/10 hover:border-emerald-400 dark:hover:border-emerald-500/40 transition-all duration-300 ${isSidebarExpanded ? 'p-2' : 'p-1 justify-center'}`}
            title={!isSidebarExpanded ? t('接入新项目') : undefined}
          >
            <div className={`shrink-0 flex items-center justify-center bg-slate-200 dark:bg-slate-800 rounded-xl group-hover:bg-emerald-500 group-hover:text-white transition-colors shadow-sm ${isSidebarExpanded ? 'w-10 h-10' : 'w-11 h-11'}`}>
              <Plus size={isSidebarExpanded ? 20 : 22} strokeWidth={2.5} />
            </div>
            <span className={`font-bold text-sm text-slate-500 dark:text-slate-400 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-all overflow-hidden whitespace-nowrap ${isSidebarExpanded ? 'ml-3 w-auto opacity-100' : 'w-0 opacity-0 ml-0'}`}>{t('接入新项目')}</span>
          </button>
        </div>
      </div>

      <div className={`w-full shrink-0 pt-4 border-t border-slate-200 dark:border-slate-800 mt-auto transition-all ${isSidebarExpanded ? 'px-4' : 'px-2 flex justify-center'}`}>
        <button
          onClick={onOpenSettings}
          className={`w-full flex items-center rounded-xl text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors group ${isSidebarExpanded ? 'p-2' : 'p-2 justify-center leading-none'}`}
        ...
        >
          ...
          <span className={`font-semibold text-sm transition-all overflow-hidden whitespace-nowrap ${isSidebarExpanded ? 'ml-3 opacity-100' : 'w-0 opacity-0 ml-0'}`}>{t('全局设置')}</span>
        </button>
      </div>
    </aside>
  );
